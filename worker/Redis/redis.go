...
func (rdbc *RedisClient) GetVideoMetadata(videoID string) (map[string]string, error) {
	key := "video:" + videoID
	return rdbc.R.HGetAll(ctx, key).Result()
}
...
func (rdbc *RedisClient) DeleteMetadata(videoID string) error {
	key := "video:" + videoID
	return rdbc.R.Del(ctx, key).Err()
}
